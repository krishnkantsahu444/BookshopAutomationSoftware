...
CREATE TABLE kk_members( emp_id int,username varchar(40),age int,address varchar(50),doj varchar(20), user_password varchar(15), PRIMARY KEY (emp_id));
ALTER TABLE kk_members add Employee_Type varchar(40);

select * from kk_members;
drop table kk_members;

select sysdate();

#employee details
insert into kk_members values( 100, 'aditya100',19,"Uttarpradesh",date(sysdate()),"aditya@100","Manager");
insert into kk_members values( 101, 'naveen101',20,"Rajsthan",date(sysdate()),"naveen@101","Employee");
insert into kk_members values( 102, 'raj102',21,"Westbengal",date(sysdate()),"raj@102","Sales Clerk");
insert into kk_members values( 103, 'kksahu123',22,"Madhyapradesh",date(sysdate()),"kksahu@123","Owner");

#manipulation command
delete from kk_members
where emp_id=102;
delete from kk_members
where emp_id=104;
drop table kk_members;
...
where customer_name="satyam sahu";
