...
def root_access():
    ...
    KK_button=Button(rootaccess,text="KK Members",relief=SUNKEN,bg="lightgreen",command=KK_Member, anchor=CENTER,padx=28,pady=9,font=(("Times New Roman",15)),borderwidth=5)
    KK_button.place(x=230,y=480)
...
def KK_access(employee_type):
    
    global KK_access_window
    global password_entry
    global user_name_entry
    
    KK_access_window=Toplevel()
    # changing the colour of main window
    KK_access_window.configure(bg="orange")
    ...
    enter_details=Label(KK_access_window,text="Enter user name and password",padx=25,pady=10,relief=SUNKEN,bg="violet")
    enter_details.place(x=315,y=350)
    
    user_name=Label(KK_access_window,text="Username",padx=15,pady=10,relief=SUNKEN)
    user_name.place(x=100,y=400)

    user_name_entry=Entry(KK_access_window,width=90,borderwidth=10)
    user_name_entry.place(x=200,y=400)

    password_label=Label(KK_access_window,text="Password",padx=15,pady=10,relief=SUNKEN)
    password_label.place(x=100,y=450)

    password_entry=Entry(KK_access_window,show="*",width=90,borderwidth=10)
    password_entry.place(x=200,y=450)
    
    
    # clear button
    clear_button1=Button(KK_access_window, text="CLEAR",bd=10, padx=50, pady=10, bg="lightgreen",command=clear)
...
def check_kk_member(employee_type):
    global user_name_entry
    global password_entry
    global KK_access_window
    sql_query="SELECT username, user_password,employee_type FROM kk_members;"
...
def KK_Member():
    ...
    sales_clerk_button=Button(KK_root,text="Sales Clerk",padx=8,pady=8,relief=SUNKEN,font=("Helvetica",14),bg="lightgreen",command=lambda: KK_access("Sales Clerk"))
    sales_clerk_button.place(x=350,y=150)
    employee_button=Button(KK_root,text="Employee",padx=15,pady=8,relief=SUNKEN,font=("Helvetica",14),bg="lightgreen",command=lambda: KK_access("Employee"))
    employee_button.place(x=350,y=230)
    manager_button=Button(KK_root,text="Manager",padx=21,pady=8,relief=SUNKEN,font=("Helvetica",14),bg="lightgreen",command=lambda: KK_access("Manager"))
    manager_button.place(x=350,y=310)
    Owner_button=Button(KK_root,text="Owner",padx=32,pady=8,relief=SUNKEN,font=("Helvetica",14),bg="lightgreen",command=lambda: KK_access("Owner"))
    Owner_button.place(x=350,y=390)
    
    main_window_button=Button(KK_root,text="Back to previous window/Page",padx=2,pady=8,relief=SUNKEN,font=("Helvetica",14),bg="green",command=lambda: backy(KK_root,rootaccess))
...
def customer_window():
    ...
    # text label-Shop name
    text_label=Label(root,text="KK Book Store",font=('Helvatical bold',50),relief=SUNKEN,bg='ivory3',anchor=CENTER,padx=50)
...
def Request_func():
    ...
    text_label_access=Label(request_win,text="KK Book Store",font=('Helvatical bold',30),relief=SUNKEN,bg='ivory3',padx=120)
...
def reading_gif():
    ...
    img=Image.open('Searching_books.gif')
...
def play_gif():
    ...
    global KK_access_window
    #global count
    load_gif=Label(KK_access_window,bg="orange")
    load_gif.grid(row=2,column=0,padx=150)
    img=Image.open('hiding_eyes3.gif')
...
connection=create_connection('localhost','root','Krishn@444','bookshop_database')
cursor=connection.cursor() 
#cursor.execute("CREATE TABLE kk_members( emp_id int,username varchar(40), password varchar(15), PRIMARY KEY (emp_id));" )

# Do the related stuff
#cursor.execute("ALTER TABLE kk_members add Employee_Type varchar(40);")
